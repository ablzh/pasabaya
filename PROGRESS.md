...
1. **Real Data Seeding:** Populate `db/seeds.rb` with real Philippine regions and cities to make the search dropdowns useful.
2. **Location UI:** Implement `TomSelect` or similar autocomplete for the city dropdowns to handle the large number of Philippine municipalities.
3. **Safety & Moderation:** Build the `Report` model and the initial Admin dashboard for community moderation.
